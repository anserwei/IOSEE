...
!
! Purpose:
!   NetCDF data I/O and file management for scientific data processing.
!   Provides convenient NetCDF read/write operations with multi-dimensional
!   array handling and comprehensive metadata management.
!
! Features:
!   - NetCDF file reading/writing operations with error handling
!   - Multi-dimensional array handling (1D through 4D)
!   - Metadata management and global attributes
!   - Memory-efficient data processing with chunked I/O
!   - Array transposition and permutation support
!   - Automatic precision handling (single/double)
!   - Comprehensive validation and error reporting
!
! Dependencies:
!   - NetCDF-Fortran: NetCDF library interface
!   - utils: Precision types
!
! Public Interface:
!   - netcdf_file: Main NetCDF file handling type
!   - open/create/close: File management operations
!   - get/put operations: Data reading/writing for scalars, vectors, matrices
!   - Attribute operations: Metadata handling
!
! Note: This module is modified from the ECMWF NetCDF utilities.
!       Original: (C) Copyright 2015-, ECMWF. All rights reserved.
!
...
!
!
module netcdf_handler

    use netcdf
    use utils,      only : jprb, jpib, jprm, jprd
...
contains
    ...
    class(netcdf_file)            :: this
        ...
        ! By default we do not transpose 2D arrays on read/write
    ...
    class(netcdf_file)           :: this
        ...
        if (istatus /= NF90_NOERR) then
            ...
            write(nulerr, '(a)') 'Error reading NetCDF file'
        end if
        ...
        if (this%is_define_mode) then
            ...
            if (istatus /= NF90_NOERR) then
                write(nulerr,'(a,a,a,a)') '*** Error ending define mode when writing ', &
                    &    trim(this%file_name), ': ', trim(nf90_strerror(istatus))
                write(nulerr, '(a)') 'Error writing NetCDF file'
            end if
            this%is_define_mode = .false.
        end if
    ...
    !---------------------------------------------------------------------
    ! Print a variable long_name, units and comment, according to
    ! verbosity level
